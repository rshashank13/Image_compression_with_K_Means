...
class K_Means():
    ...
    def move_centroid(self):
        ...
        '''
        sums = np.matrix(np.zeros(self.centroids.shape))
    ...
    def get_clusters(self,X,K,iterations,epsilon):
        ...
        for i in range(iterations):
            ...
            if self.change_greater_than(epsilon):
                ...
                break
        return self.centroids,self.iters,self.cost,self.assigned_clusters
    ...
    def run_K_means(self,X,K,iterations = 1500, epsilon = 1e-07,best_of=10):
        ...
        for i in range(best_of):
            cen,iters,coi,idx = self.get_clusters(X,K,iterations,epsilon)
            cost = self.compute_cost()
            self.models[i] = (cen,iters,coi,idx)
            costs.append(cost)
        ...
        self.assigned_clusters = self.models[ind][3]
